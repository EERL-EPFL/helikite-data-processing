...
class CO2(Instrument):
    ...
    def normalize(self, df: pd.DataFrame, verbose: bool, min_threshold: Number, max_threshold: Number) -> pd.DataFrame:
        ...
        df: DataFrame with raw data (expects specific column names)
        min_threshold: Minimum mean CO2 value required to proceed
        ...
        print(self._describe(CO2_final_moist))

        # Assign to DataFrame and mask outliers
        df[co2_moist_column_name] = CO2_final_moist
        self._remove_outliers(df, co2_moist_column_name, min_threshold, max_threshold)

        print("CO2 STP processing complete.")

        return df

    def plot_raw_and_normalized(self, df: pd.DataFrame, verbose: bool = True):
        co2_column_name = 'co2_CO2'
        co2_moist_column_name = 'co2_CO2_moist'

        plt.figure(figsize=(8, 6))
        plt.plot(df[co2_column_name], df['Altitude'], label='Measured', color='blue', marker='.', linestyle='none')
        if co2_moist_column_name in df.columns:
            plt.plot(df[co2_moist_column_name], df['Altitude'], label='Corrected', color='red', marker='.',
                     linestyle='none')
    ...
    @staticmethod
    def _remove_outliers(df: pd.DataFrame, column: str, min_threshold: Number, max_threshold: Number):
        mask = (df[column] <= min_threshold) | (df[column] >= max_threshold)
        print(
            f"{mask.sum()} outliers removed from {column} column based on thresholds {min_threshold} and {max_threshold}.")
